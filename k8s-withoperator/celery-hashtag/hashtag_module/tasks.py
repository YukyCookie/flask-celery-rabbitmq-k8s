...
# search positive
@celery_app.task(queue='hash_senti')
def search_pos(query, limit):
    STOPWORDS.update(['https', query, 't', 'co', 'will', 'amp'])
    tweets = tweepy.Paginator(client.search_recent_tweets, query="#{} -is:retweet lang:en".format(query), max_results=100).flatten(100000)
    all_tweets = []
    text = ""
    count = 0
    for tweet in tweets:
